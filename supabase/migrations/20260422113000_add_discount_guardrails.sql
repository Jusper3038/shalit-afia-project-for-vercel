...
ADD COLUMN IF NOT EXISTS minimum_profit_retention_percentage NUMERIC(5,2) NOT NULL DEFAULT 40;
...
ADD COLUMN IF NOT EXISTS unit_list_price NUMERIC(10,2) NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS discount_percentage NUMERIC(5,2) NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS discount_amount NUMERIC(10,2) NOT NULL DEFAULT 0;

UPDATE public.transactions
SET
  unit_list_price = COALESCE(NULLIF(unit_list_price, 0), unit_selling_price),
  discount_amount = COALESCE(
    NULLIF(discount_amount, 0),
    GREATEST(COALESCE(NULLIF(unit_list_price, 0), unit_selling_price) - unit_selling_price, 0)
  ),
  discount_percentage = COALESCE(
    NULLIF(discount_percentage, 0),
    CASE
      WHEN COALESCE(NULLIF(unit_list_price, 0), unit_selling_price) > 0 THEN
        ROUND(
          (
            GREATEST(COALESCE(NULLIF(unit_list_price, 0), unit_selling_price) - unit_selling_price, 0)
            / COALESCE(NULLIF(unit_list_price, 0), unit_selling_price)
          )::numeric * 100,
          2
        )
      ELSE 0
...
DECLARE
  ...
  v_minimum_profit_retention_percentage NUMERIC(5,2) := 40;
...
BEGIN
  ...
  INTO v_minimum_profit_retention_percentage
  FROM public.profiles
  WHERE user_id = auth.uid();

  v_minimum_profit_retention_percentage := COALESCE(v_minimum_profit_retention_percentage, 40);
  ...
  LOOP
    ...
    IF v_drug.selling_price <= v_drug.buying_price THEN
      v_protected_unit_selling_price := v_drug.selling_price;
    ELSE
      v_protected_unit_selling_price := ROUND(
        (
          v_drug.buying_price
          + ((v_drug.selling_price - v_drug.buying_price) * (v_minimum_profit_retention_percentage / 100.0))
        )::numeric,
        2
      );
    END IF;
