...
const TeamPage = () => {
  ...
  const toggleInviteApp = async (invite: ClinicInvite, permission: AppPermission) => {
    ...
    setInvites((current) =>
      current.map((item) =>
        item.id === invite.id ? { ...item, allowed_apps: nextApps } : item
      )
    );
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Add up to 2 users to help run this registered clinic account.
            </p>
          </div>
          <Badge variant={activeInviteCount >= 2 ? "default" : "secondary"}>
            {activeInviteCount}/2 added
          </Badge>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Share the generated link with the user. They will register with the code and join this clinic as staff.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleCreateInvite} className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
              ...
              <div className="sm:col-span-2">
                ...
                <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  {APP_PERMISSION_OPTIONS.map((option) => (
                    ...
                    >
                      <Checkbox
                        checked={selectedApps.includes(option.key)}
                        onCheckedChange={() => toggleSelectedApp(option.key)}
                        disabled={activeInviteCount >= 2}
                      />
